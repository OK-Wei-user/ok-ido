...
export function PlanPanel({ className, steps: stepsProp = [] }: PlanPanelProps) {
  ...
  // 折叠状态下优先展示当前正在执行的步骤，其次展示最后一个未完成步骤，最后回退到第一步
  const currentStep =
              ...
              <div className="max-h-[min(calc(100vh-360px),400px)] overflow-y-auto">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    step.status === 'running' ? 'text-gray-900' : 'text-gray-500'
                  )}
                >
                  {step.status === 'completed' ? (
                    <Check size={16} className="relative top-0.5 flex-shrink-0 text-green-500" />
                  ) : step.status === 'running' ? (
