...
export interface SessionHeaderProps {
  ...
  /** 打开远程桌面回调(会话活跃时显示VNC入口,便于用户手动操作如选择登录方式) */
  onOpenVNC?: () => void
  /** 会话是否活跃(running或waiting状态,控制VNC入口可见性)
  ...
  return (
    <header className="bg-[#f8f8f7] flex flex-row items-center justify-between pt-3 pb-2 gap-2 sticky top-0 z-10 flex-shrink-0">
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        {/* 远程桌面入口: 会话活跃时(running/waiting)或沙箱仍存在时可见。
            会话结束后沙箱在TTL内仍保留,用户可通过VNC查看操作结果。
            waiting状态下用户需通过VNC手动操作(如选择登录方式),必须可用。 */}
        {(isActive || hasSandbox) && onOpenVNC && (
          <Button
            variant="ghost"
            size="icon-sm"
            onClick={onOpenVNC}
