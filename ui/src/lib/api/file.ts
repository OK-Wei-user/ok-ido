...
export const fileApi = {
  ...
  downloadFile: async (fileId: string): Promise<Blob> => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://10.235.127.227:8000/api"}/files/${fileId}/download`
    );
  ...
  getFileDownloadUrl: (fileId: string): string => {
    const baseURL =
      process.env.NEXT_PUBLIC_API_BASE_URL || "http://10.235.127.227:8000/api";
