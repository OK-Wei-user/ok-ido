...
# === find_files 防护常量 ===
# 系统目录黑名单：禁止从这些目录发起递归扫描，避免 glob 进入 /proc /sys 等虚拟文件系统挂起
_FORBIDDEN_SCAN_ROOTS = frozenset({
    "/", "/proc", "/sys", "/dev", "/boot", "/run", "/snap",
    "/lib", "/lib32", "/lib64", "/libx32", "/sbin", "/bin",
})

# 递归搜索超时(秒)：** 模式扫描全树，15s 上限避免挂起
_RECURSIVE_GLOB_TIMEOUT = 15
# 非递归搜索超时(秒)：单目录列举，30s 上限
_NORMAL_GLOB_TIMEOUT = 30


def _is_forbidden_scan_root(dir_path: str) -> bool:
    """判断目录是否在系统目录黑名单中

    同时检查原始路径和 realpath 解析后的路径，
    兼容路径尾部斜杠、软链接(如 /lib -> /usr/lib)。
    """
    # 先检查原始路径(覆盖用户显式指定的系统目录)
    if dir_path in _FORBIDDEN_SCAN_ROOTS:
        return True
    # 再检查 realpath 解析后的路径(覆盖软链接指向系统目录的情况)
    try:
        real = os.path.realpath(dir_path)
    except (OSError, ValueError):
        # 路径无法解析时，按字面值比较兜底
        real = dir_path
    return real in _FORBIDDEN_SCAN_ROOTS
...
class FileService:
    ...
    async def find_files(cls, dir_path: str, glob_pattern: str) -> FileFindResult:
        ...
        防护设计：
        1. 路径验证：拒绝从系统目录(/ /proc /sys 等)发起扫描，避免 glob 进入虚拟文件系统挂起
        2. 超时保护：递归搜索(**)15s、非递归搜索30s，避免大目录扫描阻塞会话
        """
        # 1.路径验证：拒绝系统目录扫描
        if _is_forbidden_scan_root(dir_path):
            raise BadRequestException(
                f"禁止扫描系统目录[{dir_path}]，请在工作区目录(如 /home /workspace /sandbox /tmp /root)内搜索"
            )
        ...
        # 3.定义一个异步函数使用asyncio子线程运行避免IO阻塞
        def async_glob() -> List[str]:
            search_pattern = os.path.join(dir_path, glob_pattern)
            return glob.glob(search_pattern, recursive=True)

        # 4.创建子线程完成任务，并施加超时保护
        #    递归搜索(** 模式)使用 15s 上限，非递归搜索使用 30s 上限
        #    避免 glob 进入 /proc /sys 或超大目录时无限挂起
        is_recursive = "**" in glob_pattern
        timeout = _RECURSIVE_GLOB_TIMEOUT if is_recursive else _NORMAL_GLOB_TIMEOUT
        try:
            files = await asyncio.wait_for(
                asyncio.to_thread(async_glob),
                timeout=timeout,
            )
        except asyncio.TimeoutError:
            logger.warning(
                f"find_files超时(>{timeout}s): dir_path={dir_path}, pattern={glob_pattern}"
            )
            raise AppException(
                f"在[{dir_path}]中搜索[{glob_pattern}]超时(>{timeout}s)，"
                f"请缩小搜索范围或更换为更具体的目录"
            )
