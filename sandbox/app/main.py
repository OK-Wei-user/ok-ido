...
async def lifespan(app: FastAPI):
    """FastAPI生命周期上下文管理器"""
    # 1.应用开始运行之前的操作
    logger.info("I-DO沙箱正在初始化")

    try:
        setup_chinese_fonts()

        # 预加载 Python 内核常用模块(pandas/numpy/openpyxl/matplotlib)
        # fork 子进程将继承 sys.modules 缓存,避免重复 import 开销
        # 失败不阻塞启动: 内核服务内部有降级机制
        try:
            from app.services.python_kernel import get_kernel_service
            kernel = get_kernel_service()
            kernel.preload()
        except Exception as e:
            logger.warning(f"Python 内核预加载失败(降级为按需加载): {e}")

        yield
    finally:
        # 3.应用结束后的操作
        logger.info("I-DO沙箱关闭成功")
