...
class DockerSandbox(Sandbox):
    ...
    # find_files 调用专用超时(秒)：覆盖默认 read=660s，避免 sandbox glob 挂起时
    # HTTP 请求等待 11 分钟导致会话卡死。sandbox 层已有 15s/30s glob 超时兜底，
    # 此处 30s 上限作为第二层防御，给 sandbox 异常情况留余量
    _FIND_FILES_TIMEOUT = 30
    ...
    def __init__(
        ...
        """构造函数，完成Docker沙箱扩展创建"""
        self.client = httpx.AsyncClient(timeout=httpx.Timeout(connect=10.0, read=660.0, write=30.0, pool=30.0))
        self._ip = ip
    ...
    async def destroy(self) -> bool:
        ...
        资源清理分离: httpx客户端与Docker容器各自独立try块,
        确保httpx关闭失败不阻断容器清理(避免容器泄漏)。
        Docker客户端在finally中确保close(与get方法风格一致)。
        """
        # 1.关闭httpx客户端(失败不影响后续容器清理)
        if self.client:
            try:
                await self.client.aclose()
            except Exception as e:
                logger.warning(f"关闭httpx客户端失败: {str(e)}")

        # 2.关闭并移除Docker容器(确保一定执行)
        if self._container_name:
            ...
            try:
                ...
                logger.error(f"移除Docker容器[{self._container_name}]失败: {str(e)}")
                return False
            finally:
                if docker_client:
                    docker_client.close()
    ...
    async def get_browser(
        ...
        透传LLM到PlaywrightBrowser: llm用于长页面内容摘要(_CONTENT_SUMMARY开启时生效),
        multimodal_llm用于visual_click视觉兜底(均为None时浏览器仅依赖DOM五级容错,功能完整)。
        """
        return PlaywrightBrowser(self.cdp_url, llm=llm, multimodal_llm=multimodal_llm)
        ...
        for attempt in range(max_retries):
            try:
                ...
                for service in services:
                    ...
                    # 8.判断服务状态: RUNNING 正常; EXITED+exitstatus=0 视为一次性程序正常完成
                    # (如 python-warmup 预热程序, autorestart=false, 退出码 0 表示成功)
                    # 其他状态(STARTING/BACKOFF/FATAL/STOPPING/UNKNOWN 等)视为未就绪
                    is_running = state_name == "RUNNING"
                    is_completed_oneshot = state_name == "EXITED" and exit_status == 0
                    if not (is_running or is_completed_oneshot):
                        all_running = False
    ...
    async def find_files(self, dir_path: str, glob_pattern: str) -> ToolResult:
        ...
        超时保护：find_files 使用 30s 超时(覆盖客户端默认 read=660s)，
        避免 sandbox glob 操作挂起导致会话卡死。超时返回失败 ToolResult，
        让 Agent 可尝试兜底策略(如更换搜索目录、使用 shell_execute find)。
        """
        try:
            response = await self.client.post(
                ...
                timeout=self._FIND_FILES_TIMEOUT,
            )
        except httpx.TimeoutException as e:
            logger.warning(
                f"find_files HTTP 调用超时(>{self._FIND_FILES_TIMEOUT}s): "
                f"dir_path={dir_path}, pattern={glob_pattern}, error={e}"
            )
            return ToolResult(
                success=False,
                message=(
                    f"在沙箱中查找文件超时(>{self._FIND_FILES_TIMEOUT}s)："
                    f"dir_path={dir_path}, pattern={glob_pattern}。"
                    f"请缩小搜索范围或更换为更具体的目录"
                ),
            )
        return ToolResult.from_sandbox(**response.json())
    ...
    async def get_file_size(self, filepath: str) -> int:
        ...
        通过沙箱内stat命令获取,避免下载整个文件才知道大小。
        失败时返回-1,调用方按无预检查降级处理。
        """
        try:
            response = await self.client.post(
                f"{self._base_url}/api/shell/exec-command",
                json={
                    "session_id": "file_size_check",
                    "exec_dir": "/",
                    "command": f"stat -c %s {shlex.quote(filepath)} 2>/dev/null || echo -1",
                },
            )
            result = ToolResult.from_sandbox(**response.json())
            if result.success and result.data:
                output = (result.data.get("output") or "").strip()
            ...
            logger.debug(f"获取文件大小失败: filepath={filepath}, error={str(e)}")
            return -1

    async def exec_command(self, session_id: str, exec_dir: str, command: str, timeout: Optional[int] = None) -> ToolResult:
        """在沙箱中执行命令"""
        request_body = {
        ...
        try:
            ...
            logger.error(f"沙箱Shell命令执行HTTP超时: {str(e)}")
            return ToolResult(
                success=False,
                message="沙箱通信超时，命令可能仍在后台执行。请使用shell_read_output查看结果或shell_kill_process终止进程",
            )
        except httpx.ConnectError as e:
            logger.error(f"沙箱连接失败: {str(e)}")
            return ToolResult(
                success=False,
                message="沙箱连接失败，容器可能正在重启或已销毁",
            )
