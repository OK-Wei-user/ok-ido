...
_A2A_INIT_TIMEOUT = 10  # A2A服务初始化请求超时(秒)
_A2A_CARDS_CACHE_TTL = 3600  # A2A Agent卡片缓存TTL(秒),1小时
_A2A_CARDS_GLOBAL_SESSION = "__global__"  # 实例级缓存的session_id占位(A2A cards为全局共享)
...
class A2AClientManager:
    ...
    def __init__(
        ...
        Args:
            a2a_config: A2A服务配置
            prompt_cache: 会话级提示词缓存(可选),用于持久化agent cards到Redis,
                避免每次初始化都发起网络请求;None时降级为纯内存(向后兼容)
        """
        ...
        self._prompt_cache = prompt_cache  # 提示词缓存(可选,agent cards Redis持久化)
    ...
    async def initialize(self) -> None:
        ...
        优先从Redis读取缓存的agent cards(实例级共享),miss时网络获取并回写Redis,
        避免每次会话都发起网络请求。无prompt_cache时降级为纯网络获取。
        """
        # 1.检测是否已经初始化
        ...
        try:
            # 2.尝试从Redis读取缓存的agent cards(实例级共享)
            if self._prompt_cache:
                cached_cards = await self._prompt_cache.get(
                    _A2A_CARDS_GLOBAL_SESSION, "a2a_card", "all"
                )
                if cached_cards:
                    try:
                        self._agent_cards = json.loads(cached_cards)
                        logger.info(
                            f"从Redis加载A2A agent cards成功: {len(self._agent_cards)}个"
                        )
                        self._initialized = True
                        return
                    except (json.JSONDecodeError, TypeError) as e:
                        logger.warning(f"Redis缓存agent cards解析失败,降级网络获取: {e}")
            ...
            # 5.回写agent cards到Redis(实例级共享,加速后续初始化)
            if self._prompt_cache and self._agent_cards:
                try:
                    await self._prompt_cache.set(
                        _A2A_CARDS_GLOBAL_SESSION,
                        "a2a_card",
                        "all",
                        json.dumps(self._agent_cards, ensure_ascii=False),
                    )
                except Exception as e:
                    logger.warning(f"agent cards回写Redis失败(不影响主流程): {e}")
        except Exception as e:
...
class A2ATool(BaseTool):
    ...
    def __init__(self, prompt_cache: Optional[SessionPromptCache] = None) -> None:
        """构造函数，完成工具包初始化
    ...
    async def initialize(self, a2a_config: Optional[A2AConfig] = None) -> None:
        ...
        if not self._initialized:
            # 2.初始化A2A客户端管理器(透传prompt_cache用于agent cards持久化)
            self.manager = A2AClientManager(a2a_config, prompt_cache=self._prompt_cache)
            await self.manager.initialize()
            self._initialized = True

    def get_capability_hint(self) -> Optional[str]:
        """返回A2A远程Agent能力摘要,供Planner规划阶段感知A2A能力

        类似MCP的capability_hint,让Planner在规划阶段能识别可委托的远程Agent,
        在步骤描述中引用"使用call_remote_agent委托XXX Agent完成XXX"。

        Returns:
            A2A能力摘要文本,无可用Agent时返回None
        """
        if not self._initialized or not self.manager or not self.manager.agent_cards:
            return None
        lines = [
            f"[A2A远程Agent,共{len(self.manager.agent_cards)}个,"
            f"使用get_remote_agent_cards查看详情,call_remote_agent调用]"
        ]
        for agent_id, card in self.manager.agent_cards.items():
            name = card.get("name", agent_id)
            desc = (card.get("description") or "")[:80]
